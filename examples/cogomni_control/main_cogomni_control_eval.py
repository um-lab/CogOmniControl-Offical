...
from videox_fun.inference import CogOmniControl_Inferencer
...
def main(args):
    ...
    inferencer = CogOmniControl_Inferencer()
    ...
    with jsonlines.open(ouput_pred_info_path, mode=pred_info_mode, flush=True) as pred_info_writer:
        for task_id, task_info in enumerate(task_infos):
            output_task_dir = os.path.join(args.output_dir, os.path.basename(task_info["control_video"]).split(".")[0])
