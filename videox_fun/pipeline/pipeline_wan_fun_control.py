...
class WanFunControlPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # left pad for remove_ref_image
        if remove_ref_image:
            left_pad_frame_num = 4
            ref_video = torch.cat([ref_video[:, :, [0]]]*left_pad_frame_num + [ref_video], dim=2)
            control_video = torch.cat([control_video[:, :, [0]]]*left_pad_frame_num + [control_video], dim=2)
            num_frames += left_pad_frame_num
        
        # enhance a video
        if enhance_a_video:
            from third_party.Enhance_A_Video.enhance_a_video import enable_enhance, set_enhance_weight, set_num_frames
            set_num_frames(num_frames//4+1)
            enable_enhance()
        ...
        # left pad for remove ref_image
        # if remove_ref_image:
        #     latents = torch.cat([latents[:, :, [0]], latents], dim=2)  # [b, c, t, h, w]
        #     control_latents = torch.cat([control_latents[:, :, [0]], control_latents], dim=2)
        #     control_channel_num = control_latents.shape[1]
        #     control_latents[:, control_channel_num//2:, 1:] = 0
        #     num_frames += 4
        ...
        if remove_ref_image:
            video = video[:, :, left_pad_frame_num:]
