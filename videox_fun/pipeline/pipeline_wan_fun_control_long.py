        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for context_i, context_indices in enumerate(contexts):
                    ...
                    context_control_latents = control_latents[:, :, context_indices].clone()
                    if context_i != 0:
                    ...
                    with torch.cuda.amp.autocast(dtype=weight_dtype):
                        context_noise_pred = self.transformer(
                            x=latent_model_input[:, :, context_indices],
                            context=prompt_embeds,
                            t=timestep,
                            seq_len=seq_len,
                            # y=control_latents[:, :, context_indices],
                            y=context_control_latents,
                            clip_fea=clip_context,
                        )
                    noise_pred[:, :, context_indices] += context_noise_pred
                    frame_counter[:, :, context_indices] += 1

                # perform guidance
                noise_pred /= frame_counter
