...
class WanFunControlPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        ref_video: Union[torch.FloatTensor] = None,
        ...
        ref_image_strength: Optional[float] = 1.0,
        control_strength: Optional[float] = 1.0,
        v2v_strength: Optional[float] = 1.0,
        context_scheduler: str = "sequential",
        context_closed_loop: bool = False,
        context_size: int = 49,
        context_overlap: int = 9,
        ...
        if ref_video is not None:
            video_length = ref_video.shape[2]
            ref_video = self.image_processor.preprocess(rearrange(ref_video, "b c f h w -> (b f) c h w"), height=height, width=width) 
            ref_video = ref_video.to(dtype=torch.float32)
            ref_video = rearrange(ref_video, "(b f) c h w -> b c f h w", f=video_length)
            ref_video_latents = self.prepare_control_latents(
                None,
                ref_video,
                batch_size,
                height,
            ...
            timestep_num = len(self.scheduler.timesteps)
            timestep_idx = timestep_num - int(v2v_strength * timestep_num)
            ...
            def get_sigmas(timesteps, noise_scheduler, n_dim=4):
                ...
                sigmas_ = sigmas[step_indices].flatten()
                while len(sigmas_.shape) < n_dim:
                    sigmas_ = sigmas_.unsqueeze(-1)
                return sigmas_
            sigmas = get_sigmas(timestep, self.scheduler)
            latents = (1.0 - sigmas) * ref_video_latents + sigmas * noise
            num_inference_steps = int(num_inference_steps * v2v_strength)
        ...
        if control_video is not None:
            ...
            control_video_latents = control_video_latents * control_strength
            control_latents = (
        ...
        if ref_image is not None:
            ...
            if latents.size()[2] != 1:
                ref_image_latentes_conv_in[:, :, :1] = ref_image_latentes * ref_image_strength
        ...
        context_scheduler = get_context_scheduler(context_scheduler)
        latent_context_size = context_size // 4 + 1
        latent_context_stride = context_stride // 4 + 1
        latent_context_overlap = context_overlap // 4 + 1
        ...
        with self.progress_bar(total=len(contexts)*num_inference_steps) as progress_bar:
            # loop over contexts
            context_visited = set()
            for context_i, context_indices in enumerate(contexts):
                for i, t in enumerate(timesteps):
                    ...
                    latent_model_input = torch.cat([context_latents] * 2) if do_classifier_free_guidance else context_latents
                    context_control_latents = control_latents[:, :, context_indices]
                    if context_i != 0:
                        control_channel_num = context_control_latents.shape[1]
                        context_control_latents[:, control_channel_num//2:, context_indices[0]] = latents[:, :, context_indices[0]]
                    ...
                    with torch.cuda.amp.autocast(dtype=weight_dtype):
                        context_noise_pred = self.transformer(
                            x=latent_model_input,
                            context=prompt_embeds,
                            t=timestep,
                            seq_len=seq_len,
                            y=context_control_latents,
                            clip_fea=clip_context,
                        )

                    # perform guidance
                    if do_classifier_free_guidance:
                        context_noise_pred_uncond, context_noise_pred_text = context_noise_pred.chunk(2)
                        context_noise_pred = context_noise_pred_uncond + self.guidance_scale * (context_noise_pred_text - context_noise_pred_uncond)

                    # compute the previous noisy sample x_t -> x_t-1
                    # Only Update unseen latents
                    unseen_context_indices = [idx for idx in context_indices if idx not in context_visited]
                    unseen_context_flags = [idx not in context_visited for idx in context_indices] 
                    context_latents = self.scheduler.step(context_noise_pred, t, context_latents, **extra_step_kwargs, return_dict=False)[0]
                    latents[:, :, unseen_context_indices] = context_latents[:, :, unseen_context_flags]
                    ...
                    if comfyui_progressbar:
                        pbar.update(1)
                # Update context visited
                context_visited.update(context_indices)
