...
def save_video(video: np.array, path: str, fps=12, imageio_backend=True, color_transfer_post_process=False):
    outputs = []
    for x in video:
        x = x.astype(np.uint8)
        outputs.append(Image.fromarray(x))
...
def get_video_to_video_latent(input_video_path, video_length, sample_size, fps=None, validation_video_mask=None, ref_image=None, clip_image=None, video_context=None):
    if input_video_path is not None:
        if isinstance(input_video_path, str):
            ...
            while True:
                ...
                if video_context is not None and frame_count not in video_context:
                    frame_count += 1
                    continue
    ...
    clip_image = clip_image if clip_image is not None else ref_image
    if clip_image is not None:
        if isinstance(clip_image, str):
            clip_image = Image.open(clip_image).convert("RGB")
        else:
            clip_image = Image.fromarray(np.array(clip_image, np.uint8))
    else:
        clip_image = None
...
def removeprefix(text, prefix):
    ...
    except AttributeError:  # python version < 3.9
