...
@singledispatch
def broadcast_data(input_):
    input_ = [input_]
    src = nccl_info.group_id * nccl_info.sp_size
    dist.broadcast_object_list(input_, src=src, group=nccl_info.group)
    return input_[0]

@broadcast_data.register(torch.Tensor)
def broadcast_tensor(input_: torch.Tensor):
    src = nccl_info.group_id * nccl_info.sp_size
    dist.broadcast(input_, src=src, group=nccl_info.group)
    return input_

@broadcast_data.register(list)
def broadcast_object_list(input_: list):
    src = nccl_info.group_id * nccl_info.sp_size
    dist.broadcast_object_list(input_, src=src, group=nccl_info.group)
    return input_
