...
# This script extracts bf16 consolidated weights from a zero 1, 2 and 3 DeepSpeed checkpoints. It gets
# copied into the top level checkpoint dir, so the user can easily do the conversion at any point in
# the future. Once extracted, the weights don't require DeepSpeed and can be used in any
# application.
#
# example:
#   python zero_to_bf16.py . output_dir/
#   or
...
def convert_zero_checkpoint_to_bf16_state_dict(checkpoint_dir,
                                               ...
                                               exclude_frozen_parameters=False):
    """
    Convert ZeRO 2 or 3 checkpoint into a single bf16 consolidated ``state_dict`` file that can be
    loaded with ``torch.load(file)`` + ``load_state_dict()`` and used for training without DeepSpeed.

    Args:
        - ``checkpoint_dir``: path to the desired checkpoint folder. (one that contains the tag-folder, like ``global_step14``)
        - ``output_dir``: directory to the pytorch bf16 state_dict output files
...
if __name__ == "__main__":
    ...
    parser.add_argument("output_dir",
                        type=str,
                        help="directory to the pytorch bf16 state_dict output files"
