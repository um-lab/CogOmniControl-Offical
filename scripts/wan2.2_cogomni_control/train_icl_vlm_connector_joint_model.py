    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                # get llm_embeds
                # user_instruction = batch['text']
                # system_prompt = f"<|im_start|>user\n<|vision_start|><|video_pad|><|vision_end|>{user_instruction[0]}<|im_end|>\n<|im_start|>assistant\n"
                
                # user_instruction = ["Instruction: 为文本到视频生成模型分析并推理如何基于给定的控制视频（带有时间戳的图像）、参考图像（不带时间戳的图像）以及文本描述三个条件下生成最终视频。"]
                # user_instruction_no_ref = ["Instruction: 为文本到视频生成模型分析并推理如何基于给定的控制视频（带有时间戳的图像）以及文本描述生成最终视频。"]
                ...
                user_instruction_no_ref = ["Role: 你是一位精通多模态理解与视频生成的专家。\nInsturction:你的任务是协调给定的控制视频（带有时间戳的图像）以及文本描述，分析各模态中哪些核心信息需要被提取、对齐或保留。\n确保生成的视频：1. 符合高度抽象化的控制视频所暗示的潜在运动轨迹和空间结构；2. 符合文本描述。\n思考过程与方案输出必须精炼，总长度不能超过 2048 Tokens"]

                # # system_prompt = f"<|im_start|>user\n<|vision_start|><|video_pad|><|vision_end|>{user_instruction[0]}<|im_end|>\n<|im_start|>assistant\n"

                # system_prompt_image = f"<|im_start|>user\nPicture 1:<|vision_start|><|image_pad|><|vision_end|>"
                
                # system_prompt = system_prompt_image + f"\nVideo 1:<|vision_start|><|video_pad|><|vision_end|>文本描述: {batch['text'][0]}.{user_instruction[0]}<|im_end|>\n<|im_start|>assistant\n<think>"
                ...
                if ref_pixel_values.shape[1] != 0:
                    system_prompt = system_prompt + f"\nVideo 1:<|vision_start|><|video_pad|><|vision_end|>\n文本描述: {batch['text'][0]}. {user_instruction[0]}<|im_end|>\n<|im_start|>assistant\n<think>"
                else:
                    system_prompt = system_prompt + f"\nVideo 1:<|vision_start|><|video_pad|><|vision_end|>\n文本描述: {batch['text'][0]}. {user_instruction_no_ref[0]}<|im_end|>\n<|im_start|>assistant\n<think>"
                
                with torch.no_grad():
                    
                    llm_inputs = processor(
                        text = system_prompt,
                        videos = control_pixel_values[0],
                        images = [ref_pixel_values[0, i:i+1, 0, :, :] for i in range(ref_pixel_values.shape[1])] if ref_pixel_values.shape[1] != 0 else None, # [B, C, F, H, W] -> [[B, C, H, W], [B, C, H, W]]
                        return_tensors="pt",
                    ).to(device=accelerator.device)


                    # llm_embeds = llm(**llm_inputs, output_hidden_states=True).hidden_states[-1] # B, L, C
                    ...
                    # max_new_tokens = 2048
                    # B, _, D = llm_embeds.shape
                    # llm_padded = torch.zeros(B, max_new_tokens, D, device=llm_embeds.device)
                    # for i, emb in enumerate(llm_embeds):
                    #     L_i = emb.shape[0]
                    #     llm_padded[i, :L_i, :] = emb

                    # llm_embeds = llm_padded
                    ...
                    outputs = llm.generate(
                        **llm_inputs, 
                        ...
                        output_hidden_states=True)
                    # cot_steps = [step[-2] for step in outputs.hidden_states]
                    # cot_steps = [step for step in outputs.hidden_states]
                    # llm_embeds = [c for c in outputs.hidden_states[-1]]
                    # llm_embeds = torch.cat(cot_steps, dim=1)
                    # cot_steps = [step[-2] for step in outputs.hidden_states]
                    # cot_embeds = torch.cat(cot_steps, dim=1)
                    # llm_embeds = cot_embeds[:, len(llm_inputs.input_ids[-1]):, :]

                    cot_steps = [step[-2] for step in outputs.hidden_states]
                    cot_embeds = torch.cat(cot_steps, dim=1)
                    llm_embeds = cot_embeds[:, :len(llm_inputs.input_ids[-1]), :]
                    # print(len(llm_inputs.input_ids[-1]), cot_embeds.shape)
                    # print(llm_embeds.shape)
                    # llm_embeds = [u[:len(llm_inputs.input_ids)] for u in cot_embeds]
                    # llm_embeds = torch.stack(llm_embeds)
                    
                # llm_embeds.requires_grad_(True) 
