...
from videox_fun.models import (AutoencoderKLWan, CLIPModel,
                               CogOmniControlWanModel_Connector, WanT5EncoderModel)
...
from videox_fun.utils.deepspeed import deepspeed_zero3_init_disabled_context_manager
from videox_fun.utils.lora_utils import merge_lora_to_modules
from videox_fun.utils.llm_utils import build_llm_embeds
...
def parse_args():
    ...
    parser.add_argument(
        "--pretrained_llm_model_name_or_path",
        type=str,
        default=None,
        required=True,
        help="Path to pretrained model or model identifier from huggingface.co/models.",
    ...
    parser.add_argument(
        "--pretrained_llm_lora_model_name_or_path",
        type=str,
        default=None,
        help="Path to pretrained model or model identifier from huggingface.co/models.",
    )
    parser.add_argument(
        "--llm_embed_layer_indices",
        type=str,
        default=None,
        help=(
            "指定用于构建 llm_embeds 的层序号列表（逗号分隔，如 '16,24,36'）。"
            "0 表示 embedding 层输出，1~num_hidden_layers 表示各 decoder layer 输出，"
            "也支持负数索引（如 -1 表示最后一层）。若指定多个层，会分别取出每一层的完整 "
            "(prompt + CoT answer) 序列，再沿序列长度维（seq）依次拼接，特征维（hidden_size）"
            "保持不变，因此无需调整 config 中 transformer 的 text_dim。默认为 None，"
            "即只取最后一层，与原始行为保持一致。"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--connector_path", type=str, default=None, help="lora path."
    )
...
def main():
    ...
    # 解析用户指定的 llm_embeds 取层列表，例如 "0,17,35" -> [0, 17, 35]。
    # 未指定时保持 None，build_llm_embeds 内部会回退为只取最后一层。
    llm_embed_layer_indices = None
    if args.llm_embed_layer_indices is not None and len(args.llm_embed_layer_indices.strip()) > 0:
        llm_embed_layer_indices = [
            int(idx.strip()) for idx in args.llm_embed_layer_indices.split(",") if idx.strip() != ""
        ]
    ...
    dist_timeout = timedelta(seconds=int(os.getenv("DIST_TIMEOUT_SEC", "7200")))
    accelerator = Accelerator(
        gradient_accumulation_steps=args.gradient_accumulation_steps,
        mixed_precision=args.mixed_precision,
        log_with=args.report_to,
        project_config=accelerator_project_config,
        kwargs_handlers=[InitProcessGroupKwargs(timeout=dist_timeout)],
    )
    ...
    accelerator.print(f"Init rng with seed {args.seed + accelerator.process_index}. Process_index is {accelerator.process_index}")
    ...
    with ContextManagers(deepspeed_zero3_init_disabled_context_manager()):
        # Get Text encoder
        ...
        llm = Qwen3VLForConditionalGeneration.from_pretrained(
                args.pretrained_llm_model_name_or_path,
                low_cpu_mem_usage=True,
                attn_implementation="flash_attention_3",
                torch_dtype=weight_dtype,
            )
        if args.pretrained_llm_lora_model_name_or_path is not None:
            from peft import PeftModel
            accelerator.print("Loading LLM Reasoning Lora: ", args.pretrained_llm_lora_model_name_or_path)
            llm = PeftModel.from_pretrained(
                llm, 
                args.pretrained_llm_lora_model_name_or_path
            )
        llm = llm.eval()
    ...
    transformer3d = CogOmniControlWanModel_Connector(
        **filter_kwargs(CogOmniControlWanModel_Connector, OmegaConf.to_container(config["transformer_additional_kwargs"]))
    ).to(weight_dtype)
    enable_control_ref_attention_dropout(
        transformer3d,
        drop_control_prob=0.2,   # dropout 0.1 Control latent
        drop_ref_prob=0.2,       # dropout 0.1  Ref latent
        drop_together=True,      # True: control/ref dropout together
    ...
    # uncomment this for a fast debug
    ...
    network = create_network(
        ...
        skip_names=["head", "text_embedding", "time_embedding", "time_projection", "llm_fea_connector"],
    )
    network.apply_to(
        text_encoder, transformer3d, False and not args.training_with_video_token_length, True, True
    )

    accelerator.print("load lora from ", args.lora_path)
    network.load_weights(args.lora_path)
    accelerator.print("load connector from ", args.connector_path)
    network.load_connector(args.connector_path)


    if args.vae_path is not None:
        accelerator.print(f"From checkpoint: {args.vae_path}")
        ...
        if not zero_stage == 3:
            ...
            def load_model_hook(models, input_dir):
                ...
                for i in range(len(models)):
                    ...
                    load_model = CogOmniControlWanModel_Connector.from_pretrained(
    ...
    trainable_params_optim = network.prepare_joint_optimizer_params(
    ...
    llm.to(accelerator.device if not args.low_vram else "cpu", dtype=weight_dtype)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.enable_text_encoder_in_dataloader:
                    ...
                    # prompt_embeds = [u for u in prompt_embeds]
                ...
                with torch.no_grad():
                    ...
                    llm_inputs = processor(
                        text = system_prompt,
                        videos = (((control_pixel_values[0] + 1) / 2.0) * 255.0) if control_pixel_values.shape[1]!=1 else (((torch.cat([control_pixel_values, control_pixel_values], dim=1)[0] + 1) / 2.0) * 255),
                        # images = [(((ref_pixel_values[0, i:i+1, :, :, :] + 1) / 2.0) * 255) for i in range(ref_pixel_values.shape[1])] if ref_pixel_values.shape[1] != 0 else None, # b f c h w
                        images = [((ref_pixel_values[0, i] + 1.0) / 2.0) * 255.0 for i in range(ref_pixel_values.shape[1])] if ref_pixel_values.shape[1] != 0 else None,
                        return_tensors="pt",
                        video_metadata = {
                            "fps": 12,
                            "total_num_frames": control_pixel_values.shape[1] if control_pixel_values.shape[1]!=1 else control_pixel_values.shape[1] * 2,
                            } 
                    ...
                    # llm_embeds = torch.cat(cot_steps, dim=1)
                    # 按 --llm_embed_layer_indices 指定的层列表，从 LLM 的多层 hidden_states
                    # 中取出对应层并在特征维（hidden_size）上 concat，构成 deepstack 特征。
                    # 若未指定（None），则保持原始行为，只取最后一层。
                    llm_embeds, cut_idx = build_llm_embeds(
                        outputs, llm_inputs['input_ids'], processor.tokenizer,
                        marker_text="# Evaluator Registry（评估器库）", 
                        keep_marker=False,
                        # layer_indices=llm_embed_layer_indices,
                        layer_indices=[16, 24, -1],
                        )
                    accelerator.print(control_pixel_values[0].shape, ref_pixel_values[0].shape, llm_inputs.pixel_values_videos.unsqueeze(0).shape, llm_inputs.pixel_values.unsqueeze(0).shape, llm_embeds.shape)

                    # === 对齐 SP 组内 llm_embeds 长度 ===
                    # llm.generate 为自回归生成，即使 SP 组内输入相同，采样/浮点
                    # 非确定性也会导致各 rank 生成的 CoT 长度不一致，进而使下游
                    # seq_len 不同，触发 connector 中 all_to_all_4D 的 size
                    # mismatch，最终 NCCL 集合通信超时挂死（collective timeout）。
                    # 这里统一广播 SP 组首 rank 的 llm_embeds，保证组内长度一致。
                    if args.sp_size > 1:
                        llm_len_t = torch.tensor(
                            [llm_embeds.shape[1]], device=accelerator.device, dtype=torch.long
                        )
                        llm_len_t = broadcast_data(llm_len_t)
                        target_llm_len = int(llm_len_t.item())
                        b_, cur_len, d_ = llm_embeds.shape
                        llm_buf = torch.zeros(
                            (b_, target_llm_len, d_),
                            device=llm_embeds.device,
                            dtype=llm_embeds.dtype,
                        )
                        if nccl_info.rank_within_group == 0:
                            copy_len = min(target_llm_len, cur_len)
                            llm_buf[:, :copy_len] = llm_embeds[:, :copy_len]
                        llm_embeds = broadcast_data(llm_buf)
